...
## 1. 제품 별 품질타입 L,M,H(각각 50,30,20%)의 차이 분석 요약: 
# 제품 별 품질타입 L,M,H에 따라 평균에 유의미한 차이를 보이는 변수가 없었음.
...
## 2. 양호/고장발생 그룹간 차이 분석 요약:
# Air_temperature: 평균차이O, 분산차이X
# Process_temperature: 평균차이O, 분산차이O
# Rotational_Speed: 평균차이O, 분산차이O
# Torque: 평균차이O, 분산차이O
# Tool wear: 평균차이O, 분산차이O
# 거의 모든 변수에서 양호/고장발생 그룹간 평균 차이가 있었고, 분산 차이도 있었음.
...
#Failure_Type 삭제
#Model Train 과정에서 해당 Feature를 기준으로 결과를 예측하게되므로 제거 진행.
dat1 <- dat[,-c(8)]
unique(dat$Target)

# 트레인, 테스트셋 분할
set.seed(0)
trainidx <- sample(1:nrow(dat1), 0.7*nrow(dat1))
trainset <- dat1[trainidx,]
testset <- dat1[-trainidx,]


# 표준화
# Target을 종속 변수로 설정
scaling <- preProcess(trainset, method = c("center", "scale"))

trainsc <- predict(scaling, trainset)

testsc <- predict(scaling, testset)
...
trainpca1 <- cbind(trainpca1, Target = trainsc1[, 'Target'])

testpca1 <- predict(pca1, testsc1[,-ncol(testsc1)])[,1:4]
testpca1 <- cbind(testpca1, Target = testsc1[, 'Target'])
...
# PCA 데이터보다 원본 데이터를 사용한 모델이 더 나은 성능을 보임
# pca 데이터셋 모델이 MAE 값에서 약간 더 낮은 값을 보이나 mse와 rsme 값에서는 원 데이터셋 모델이 더 낮은 값을 보임
# reg1,reg1_1,reg1_2,reg1_3 -> 모든 모델이 거의 동일한 값을 가짐 (MAE:0.417 MSE:0.859 RMSE: 0.927)
# pca를 통하여 차원을 줄였으나 오히려 성능을 저하는 시키는 결과를 야기함 
...
# 결과:
# 원 데이터셋에서는 Ridge 및 Elastic Net 모델이 약간 더 좋은 성능을 보였지만, 성능 차이는 매우 미미함
# PCA 데이터셋에서는 모든 모델의 성능이 동일하였지만 원데이터셋 모델보다 성능이 저조하였음
...
svr <- train(Target ~ ., data = trainsc1, method = 'svmRadial',
...
evaluate(testsc1$Target, pred_svr)
